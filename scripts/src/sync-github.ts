...
const MAX_FILES = 1000;                        // arbitrary sanity cap
...
async function main() {
  ...
  // Always operate from the git repo root. pnpm --filter runs this script
  // with CWD = scripts/, which causes git status/ls-files to scope changes
  // to that subtree and silently drop everything outside (README.md,
  // launch.*, artifacts/, lib/, replit.md, etc.).
